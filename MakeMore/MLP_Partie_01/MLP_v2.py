...
# Version en augmentant la taille des données
...
# Ou emb.shape[0]
#Et on utilise la fonction d'activation tanh
h = torch.tanh(emb.view(-1, 6) @ W1 + b1) # On a maintenant un tensor de taille 32, 100
...
for i in range(20000):
    ...
    # Forward pass
    emb = C[X[ix]] # embedder les inputs selon mini-batch
    ...
    # Update
    # lr = lrs[i]
    lr = 10 ** -1.2 # learning rate optimal
    for p in parameters:
...
# plt.plot(lri, lossi)
# plt.show()
# On determine l'exposant en prenant la partie de la courbe le plus bas et stable
